...
test("regex", () => {
  const expected = 42
  const actual = match("hello world")(
    [/hello/, () => expected],
    [_, () => "nope"]
  )
...
test("multiple spread arrays", () => {
  ...
  const actual = match(["start", 1, 2, 3, "bingo", "bongo", "bango", "bunga"])(
    [
      ["start", ...type(Number), "bingo", ...type(String), "bunga"],
      () => expected,
    ],
    [_, () => "nope"]
  )
